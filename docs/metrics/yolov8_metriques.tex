...
YOLOv8 (\textit{You Only Look Once version 8}) est
...
\begin{examplebox}{Comparaison des modèles YOLOv8}
  ...
  \end{tabular}
  \end{center}
  \textit{Source : benchmarks publics YOLOv8, GPU Tesla T4, image 640$\times$640.}
\end{examplebox}
...
# ──────────────────────────────────────────────────────────────────────────────
# 4. Entraînement YOLOv8 avec ce dépôt
# ──────────────────────────────────────────────────────────────────────────────
#   trainyolo8 --config gpu/configs/train.yaml
#
# La configuration (dataset, epochs, image_size, batch_size, device...)
# se fait dans le fichier YAML. Les courbes, l'historique et les poids
# sont écrits dans runs/<run_name>/train/.

# 5. Évaluation
#   evalyolo8 --config gpu/configs/eval.yaml
#
# Les métriques finales (mAP50, mAP50-95, precision, recall, F1,
# matrice de confusion, tables par classe) sont écrites dans
# runs/<run_name>/eval/ (results.csv, per_class.csv, plots).
\end{lstlisting}
